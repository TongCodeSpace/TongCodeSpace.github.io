...
### 自动化方案
1. 在应用启动时，通过 Lua 脚本去 Redis 获取标识位。DataCenterId 和 workerId 的获取与自增在 Lua 脚本中完成，调用返回后就是可用的标示位。[lua 脚本雪花算法](lua%20脚本雪花算法.md)但是引入了一个新的组件后，将会有新的风险，当 redis 宕机后怎么办
